<template :auth="auth" :window="window">
  ...
  <div class="mx-5 flex justify-between">
    ...
    <Link href="doctors/register" class="rounded p-2 font-semibold text-lg bg-sky-600 text-white hover:bg-sky-500 hover:cursor-pointer">Register</Link>
  </div>
      ...
      <table class="w-full table-auto border-2 shadow">
        ...
        <tbody>
          <tr v-for="(doctor, i) in doctors" :key="i">
            ...
            <td class="p-3">{{doctor.contact_no}}</td>
            <td class="p-3">{{doctor.email}}</td>
            <td class="p-3">{{doctor.username}}</td>
            <td class="p-3">{{doctor.status}}</td>
            <td class="p-3">
  ...
  <template v-else>
    <div class="m-6 pt-6 flex flex-col justifty-center items-center">
        ...
        <p class="text-3xl">No registered doctors yet.</p>
    </div>
  </template>
</template>
